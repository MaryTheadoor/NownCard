...
import { isLightBg } from "@/shared/lib/utils/helpers";
import type { Card } from "@/shared/api/types";

export interface CardThemeResult {
  isDark: boolean;
  accent: string;
...
export function useCardTheme(card: Partial<Card>): CardThemeResult {
  const isDark = useMemo(() => {
    if (card.cardBgColor) return !isLightBg(card.cardBgColor);
    return card.cardTheme === "dark";
  }, [card.cardBgColor, card.cardTheme]);

  const customBg = card.cardBgColor || undefined;
  const accent = card.accentColor || "#d4a34a";
  const primaryTextColor =
    card.textColor || (isDark ? "#f4f1ec" : "#1a1612");
  const textColorStyle = card.textColor
    ? { color: card.textColor }
    : undefined;
  const bgOpacity = card.bgOpacity ?? (isDark ? 0.6 : 0.3);
  const bgSizeStyle = card.bgZoom
    ? card.bgZoom + "%"
    : card.bgSize || "cover";

  const profileSizePx =
    card.profileSize === "small" ? 56 : card.profileSize === "large" ? 88 : 72;
  const profileShapeClass =
    card.profileShape === "rounded"
      ? "rounded-2xl"
      : card.profileShape === "square"
        ? "rounded-none"
        : "rounded-full";
  const profileFontSize =
    card.profileSize === "small"
      ? 18
      : card.profileSize === "large"
        ? 26
        : 22;
  ...
  const tc = useMemo(
    () => ({
      ...
      faceShadow: isDark
        ? "0 1px 0 rgba(255,255,255,0.05) inset, 0 24px 60px rgba(0,0,0,0.6), 0 4px 12px rgba(0,0,0,0.4)"
        : undefined,
      ...
      socialHoverBg: isDark
        ? "hover:bg-white/5"
        : "hover:bg-[rgba(42,37,32,0.06)]",
      socialHoverText: isDark
        ? "hover:text-[#e8e4de]"
        : "hover:text-[#2a2520]",
      qrSub: isDark ? "text-[#9a9186]" : "text-[#7a7166]",
      profileFallbackBg: isDark
        ? "bg-gradient-to-br from-[#2a2a3a] to-[#1a1a2e]"
        : "bg-gradient-to-br from-[#d4cfc8] to-[#e8e4de]",
